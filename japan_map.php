...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // 写真最大10枚
    $validPhotoCandidates = array_values(array_filter(
        $photoFiles,
        ...
        try {
            ...
            */
            ...
            /* ④ 写真保存（uploads + route_photos）
               ✅ 写真が1枚も無いならフォルダ作成もしない（=環境の権限問題を回避）
            */
            if (count($validPhotoCandidates) > 0) {
                ...
                foreach ($validPhotoCandidates as $file) {
                    if (($file['error'] ?? 0) !== UPLOAD_ERR_OK) {
                        throw new RuntimeException('写真アップロードに失敗したファイルがあります');
                    }
                    if (!is_uploaded_file($file['tmp_name'])) {
                        throw new RuntimeException('不正なアップロードが検出されました');
                    }

                    $ext = safe_image_extension($file['tmp_name']);
                    if ($ext === null) {
                        throw new RuntimeException('対応していない画像形式があります（jpg/png/webpのみ）');
                    }
            ...
            // ✅ 原因調査のため、一旦メッセージを表示する（解決後に文言を戻してOK）
